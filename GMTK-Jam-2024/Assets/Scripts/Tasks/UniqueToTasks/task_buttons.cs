...
public class task_buttons : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        numberUntilDone = buttonList.Count;
    }
    ...
    */
    ...
    private int numberUntilDone;

    private float numberCompleted = 0;
    ...
    private void Update()
    {
        ...
        {
            canPress = true;
            task.SetAmountTaskComplete((float)(numberCompleted/(float)numberUntilDone));

            for (int i = 0; i < buttonList.Count; i++)
            {
                numberCompleted = ButtonTotalCalcuation();
            }

            if (numberCompleted == numberUntilDone)
            {
                task.TaskSolved();
                ResetTask();
            }
    ...
    void ResetTask()
    {
        ...
        for (int i = 0; i < buttonList.Count; i++)
        {
            buttonList[i].GetComponent<ButtonIsPressed>().isPressed = false;
        }
    }
    ...
    private int ButtonTotalCalcuation()
    {
        int numbPressed = 0;

        for (int i = 0; i < buttonList.Count; i++)
        {
            if (buttonList[i].GetComponent<ButtonIsPressed>().isPressed)
            {
                numbPressed++;
            }
        }
